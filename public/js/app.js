const App = (function () {
  ...
  function timelineTemplate(data) {
    const tmpl = '<li class="collection-item">' + '<p class="title">' + data.title + "</p>" + '<p class="author">Author: ' + data.userId + "</p>" + '<div class="body">' + data.body + "</div>" + "</li>";
  ...
  function timeline() {
    ...
    $.ajax({
      ...
      .done(function (data) {
        if (!data) {
          preloader.hide().find("> div").removeClass("active");
